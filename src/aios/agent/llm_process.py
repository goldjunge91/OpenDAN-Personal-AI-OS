...
class BaseLLMProcess(ABC):
    ...
    async def _execute_inner_func(self, inner_func_call_node: Dict, prompt: LLMPrompt,
                                  stack_limit=1) -> ComputeTaskResult:
        arguments = None
    ...
    async def process(self, input: Dict) -> LLMResult:
        ...
        task_result: ComputeTaskResult = await (ComputeKernel.get_instance().do_llm_completion(
            ...
            max_token=max_result_token,
            inner_functions=prompt.inner_functions,
            # NOTICE: inner_function in prompt can be a subset of get_inner_function
            timeout=self.timeout))
...
class LLMAgentBaseProcess(BaseLLMProcess):
    ...
    def prepare_knowledge_grpah_prompt(self) -> Dict:
        ...
        if self.memory.enable_knowledge_graph:
            result["knowledge_graph_list"][
                self.memory.knowledge_graph.kb_id] = self.memory.knowledge_graph.get_description()
            have_kb = True
...
class AgentMessageProcess(LLMAgentBaseProcess):
        ...
        if msg.is_image_msg():
            if self.enable_media2text:
                ...
                if image_prompt is None:
                    msg_prompt.messages = [{"role": "user", "content": [
                        {"type": "image_url", "image_url": {"url": self.check_and_to_base64(image)}} for image in
                        images]}]
                else:
            ...
            if self.enable_media2text:
                ...
                if self.asr_model is not None:
                    resp = await (
                        ComputeKernel.get_instance().do_speech_to_text(audio_file, model=self.asr_model, prompt=None,
            ...
            if self.enable_media2text:
                ...
                if resp.result_code != ComputeTaskResultCode.OK:
                    ...
                    if prompt == "":
                        ...
                        msg_prompt.messages = [{"role": "user", "content": prompt},
                                               {"role": "user", "content": f"Voice content:{resp.result_str}"}]
        else:
    ...
    async def prepare_prompt(self, input: Dict) -> LLMPrompt:
        ...
        prompt.inner_functions = LLMProcessContext.aifunctions_to_inner_functions(
            self.llm_context.get_all_ai_functions())
        if self.workspace:
