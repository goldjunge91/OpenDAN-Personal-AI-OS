...
class ChatSessionDB:
    ...
    def insert_message(self, msg: AgentMsg, tags: List[str] = None):
        ...
        try:
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?,?)
            """, (
            msg.msg_id, msg.session_id, msg.msg_type.value, msg.prev_msg_id, msg.sender, msg.target, msg.create_time,
            msg.topic, mentions, msg.body_mime, msg.body, action_name, action_params, action_result, msg.done_time,
            msg.status.value, str_tags))
            conn.commit()
    ...
    def get_message_by_id(self, message_id):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT MessageID, SessionID, MsgType, PrevMsgID, SenderID, ReceiverID, Timestamp, Topic,Mentions,ContentMIME,Content,ActionName,ActionParams,ActionResult,DoneTime,Status FROM Messages WHERE MessageID = ?",
            (message_id,))
        message = c.fetchone()
        return message
...
class AIChatSession:
    ...
    def load_message_records_by_agentid(cls, agent_id: str, start_time: str, limit: int, db_path: str) -> List[
        AgentMsg]:
        db = cls._dbs.get(db_path)
        if db is None:
            db = ChatSessionDB(db_path)
