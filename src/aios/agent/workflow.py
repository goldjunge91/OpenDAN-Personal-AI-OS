...
class Workflow:
    ...
    async def _forword_msg(self, inner_obj_id, msg):
        ...
        while i < len(inner_obj_id):
            if i == len(inner_obj_id) - 1:
                ...
                current_workflow_chatsession = AIChatSession.get_session(current_workflow.workflow_id,
                                                                         msg.sender + "#" + msg.topic,
                                                                         current_workflow.db_file)
    ...
    async def _process_msg(self, msg: AgentMsg) -> AgentMsg:
        ...
        if msg.mentions is not None:
            ...
            error_resp = msg.create_error_resp(
                f"workflow {self.workflow_id} recv a group chat message,not support ignore!")
            return error_resp
        ...
        if msg.mentions is not None:
            if not self.workflow_id in msg.mentions:
                ...
                logger.info(
                    f"workflow {self.workflow_id} recv a group chat message from {msg.sender},but is not mentioned,ignore!")
                return None
    ...
    async def _role_execute_func(self, the_role: AIRole, inenr_func_call_node: dict, prompt: LLMPrompt,
                                 org_msg: AgentMsg, stack_limit=5) -> [str, int]:

        func_name = inenr_func_call_node.get("name")
        arguments = json.loads(inenr_func_call_node.get("arguments"))
        ineternal_call_record = AgentMsg.create_internal_call_msg(func_name, arguments, org_msg.get_msg_id(),
                                                                  org_msg.target)
        ...
        task_result: ComputeTaskResult = await ComputeKernel.get_instance().do_llm_completion(prompt,
                                                                                              the_role.agent.llm_model_name,
                                                                                              the_role.agent.max_token_size,
                                                                                              inner_functions)
    ...
    async def role_process_msg(self, msg: AgentMsg, the_role: AIRole, workflow_chat_session: AIChatSession) -> AgentMsg:
        ...
        async def _do_process_msg():
            ...
            task_result: ComputeTaskResult = await ComputeKernel.get_instance().do_llm_completion(prompt,
                                                                                                  the_role.agent.get_llm_model_name(),
                                                                                                  the_role.agent.get_max_token_size(),
                                                                                                  inner_functions)
            if task_result.result_code != ComputeTaskResultCode.OK:
                logger.error(f"llm compute error:{task_result.error_str}")
            ...
            for postmsg in result.post_msgs:
                ...
                if not self._is_in_same_workflow(postmsg):
                    role_sesion = AIChatSession.get_session(the_role.get_role_id(), f"{postmsg.target}#{msg.topic}",
                                                            self.db_file)
                ...
                case "waiting":
                    for sendmsg in result.send_msgs:
                        ...
                        if not self._is_in_same_workflow(sendmsg):
                            role_sesion = AIChatSession.get_session(the_role.get_role_id(),
                                                                    f"{sendmsg.target}#{sendmsg.topic}", self.db_file)
