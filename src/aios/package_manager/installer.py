...
class PackageInstaller:
    ...
    def install(self, pkg_name: str,
                install_from_dependency=False, can_upgrade=True, skip_depends=False, options=None) -> Tuple[
        PackageInstallTask, str]:
        ...
        logger.info(
            f"start download&install {pkg_name},install_from_dependency={install_from_dependency},upgrade={is_upgrade},backup={need_backup},target_pkg_info={the_pkg_info}")
        ...
        async def download_and_install_pkg() -> int:
            ...
            if is_download_directy is False:
                ...
                install_media_result = await self.owner_env.do_pkg_media_trans(the_pkg_info, target_full_path,
                                                                               install_full_path)
